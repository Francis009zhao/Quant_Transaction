...
def init(context):
    ...
    for index in range(15, (len(days) - 5)):
        ...
        vol = volume[-1] #xian.liang
        return_now = close[-1] / close[0]   #区间收益率
        std = np.std(np.array(close), axis=0) # 区间标准差,axis=0:按列求标准差
        #用features来存放特征因子，并加到train
    ...
    #训练svm
    context.clf = svm.SVC(C=1.0, kernel='rbf', degree=3, gamma='auto', coef0=0.0,

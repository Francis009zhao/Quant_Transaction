...
def init(context):
    ...
    # 用于记录工作日，获取标的的daily历史行情
    recent_data = history(context.symbol, frequency='1d', start_time=start_date,
    ...
    for index in range(15, (len(days)-5)):
        ...
        end_day = days[index]
        data = history(context.symbol, frequency='1d', start_time=start_day, end_time=end_day, fill_missing='last', df=True)
...
def on_bar(context, bars):
    bar = bars[0]
    # 获取当前年月日
    today = bar.bob.strftrime('%Y-%m-%d')
    # 获取数据并计算相应的因子，于星期一的9点31进行操作，
    # 当前bar的工作日
    ...
    if not position and weekday == 1:
        # 获取预测用的历史数据
        data = history_n(symbol=context.symbol, frequency='1d', end_time=today, count=15, fill_missing='last', df=True)
        close = data['close'].values
        ...
        # 若预测值为上涨则开仓
        if prediction == 1:
            context.price = close[-1]   # 获取昨日收盘价
            order_target_percent(symbol=context.symbol, percent=0.95,
            ...
            order_close_all()   # 即清掉所有仓位
        # 当时间为周五并且跌幅大于-2%时，平掉所有仓位进行止损
...
if __name__ == '__main__':
    ...
    '''
    run(strategy_id='358119b1-baa8-11e9-ae6b-3c2c3098973f',
        filename='main.py',
        mode=MODE_BACKTEST,

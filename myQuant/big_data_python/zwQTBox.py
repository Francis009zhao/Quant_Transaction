...
import pandas as pd
from myQuant.big_data_python import data as web
...
from myQuant.big_data_python import zwSys as zw  #::zwQT
from myQuant.big_data_python import zwTools as zwt
...
def xtick2tim_code010(qx):
    ...
    for kss in qx.min_ksgns: 
        min_knum,min_ksgnWrk=int(kss),'M'+kss
...
def xedit_zwXDat(df):
    ...
    df['z-xdiff']=df['xdiff']*1000/dx;df['z-xdiff']=df['z-xdiff'].round(0)
    df['z-open']=df['open']*1000/dx.shift(1);df['z-open']=df['z-open'].round(0)
    df['z-high']=df['high']*1000/dx;df['z-high']=df['z-high'].round(0)
    df['z-low']=df['low']*1000/dx;df['z-low']=df['z-low'].round(0)
    df['z-close']=df['close']*1000/dx;df['z-close']=df['z-close'].round(0)
